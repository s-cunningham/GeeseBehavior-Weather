...
cat("
    ...
    for (i in 1:nind) {
      ad[i] ~ dbern(p[i])
      logit(p[i]) <- beta0 + beta1*antPTF[i] + beta2*year1[i] 
    }
...
# Bundle data
jags.data <- list(ad=dat2[,5], nind=nind, un.dur=un.dur, ptf=Y, year1=dat2[,7])

# Initial values
inits <- function() {list(beta0=rnorm(1), beta1=rnorm(1), beta2=rnorm(1), delta=rep(1,un.dur), sig=1)}

# Parameters monitored
params <- c("beta0", "beta1", "beta2", "p", "delta", "sig", "antPTF", "weight", "antX1", "weightOrdered", "cum.weight")
...
# Run model
out2p <- jags(jags.data, inits, params, "R/ecomem_1.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, parallel=TRUE)

print(out2p, digits=3)

range(out2$Rhat)

smry <- as.data.frame(out2$summary)
write.csv(smry, "output/modelcoefficients/PTF_ecomem_NAMC3.csv")

save(file="output/ecomem_NAMC3ptf_priors.Rdata", list="out2p")
...
cat("
    ...
    # for (i in 1:nind) {
      ad[i] ~ dbern(p[i])
      logit(p[i]) <- beta0 + beta1*antODBA[i] + beta2*year1[i]
    }
...
# Bundle data
jags.data <- list(ad=dat2[,5], nind=nind, un.dur=un.dur, odba=Y, year1=dat2[,7]) 

# Initial values
inits <- function() {list(beta0=rnorm(1), beta1=rnorm(1), beta2=rnorm(1), delta=rep(1,un.dur), sig=1)}

# Parameters monitored
params <- c("beta0", "beta1", "beta2", "delta", "sig", "antODBA", "weight", "antX1", "weightOrdered", "cum.weight")
...
# Run model
out1p <- jags(jags.data, inits, params, "R/ecomem_2.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, parallel=TRUE)

print(out1p, digits=3)

range(out1$Rhat)

smry <- as.data.frame(out1$summary)
write.csv(smry, "output/modelcoefficients/ODBA_ecomem_NAMC3.csv")

save(file="output/ecomem_NAMCodba3_priors.Rdata", list="out1p")
...
# Run model
out4p <- jags(jags.data, inits, params, "R/ecomem_1.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, parallel=TRUE)

print(out4p, digits=3)
...
save(file="output/ecomem_GRLD3ptf_priors.Rdata", list="out4p")
...
# Run model
out3p <- jags(jags.data, inits, params, "R/ecomem_2.txt", n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, parallel=TRUE)

print(out3p, digits=3)
...
save(file="output/ecomem_GRLDodba3_priors.Rdata", list="out3p")
