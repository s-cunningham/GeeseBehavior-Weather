...
comb.ss %>% group_by(behavior) %>% count()
...
# Random Forest 
K=10
folds <- sample(1:K, nrow(comb.ss), replace=TRUE)
...
for (i in 1:K) {
  comb.train <- comb.ss[folds != i,2:39]
  behavior <- comb.ss[folds != i, 1]
  ot <- cbind(behavior, comb.train)
  comb.test <- comb.ss[folds == i,2:39]
  test.behaviors <- comb.ss[folds==i,1]
...
KCV=10
# folds <- sample(1:KCV, nrow(comb.ss), replace=TRUE)
...
for (i in 1:KCV) {
  comb.train <- comb.ss[folds != i,2:39]
  train.behaviors <- comb.ss[folds != i, 1]
  comb.test <- comb.ss[folds == i,2:39]
  test.behaviors <- comb.ss[folds==i,1]
