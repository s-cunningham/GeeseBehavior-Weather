...
ggplot(d2, aes(x=julian, y=value, color=variable, group=animal_id)) + geom_point() 
...
cat("
    ...
    a[1] <- mu[1] * phi
    b[1] <- (1-mu[1]) * phi
    logit(mu[1]) <- beta0 + beta1[1]*prate[1] + beta2[1]*temp[1]
    ...
    for (j in 2:dur) {
      ...
      a[j] <- mu[j] * phi
      b[j] <- (1-mu[j]) * phi
      logit(mu[j]) <- beta0 + beta1[j]*prate[j] + beta2[j]*temp[j]
...
for (i in 1:nrow(dr)) {
  ...
  # Run model
  out1 <- jags(jags.data, inits, params, "R/dlm_prop.odba_ind.txt", n.adapt=na, n.thin=nt, n.chains=nc, n.burnin=nb, n.iter=ni, parallel=TRUE)
  
  whiskerplot(out1, parameters="beta1")
