...
ggplot(dat, aes(y=dist, x=julian, group=key)) + geom_point() +
  geom_vline(xintercept=c(153, 183)) + 
  geom_vline(xintercept=c(207, 210), color="red") + 
...
# Rule 2: Fewer than 12 days of movement greater than 4*SD of molt movement (~2km) in 25 day period
...
for (i in 1:length(un.id)) {
  ...
  temp2 <- as.numeric(temp$V2)<=9  # Number of days (12) or 5
...
# Rule 3: ODBA stays low for 25 days (with less than 5 large movements) 
...
for (i in 1:length(un.id)) {
  ...
  if (nrow(temp)>1) {
    ...
    temp2 <- as.numeric(temp$V2)<=7  # Is this only 5 days?
    temp <- cbind(temp, temp2)
